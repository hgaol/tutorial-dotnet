...
namespace Playground.Identity
{
    public class ClientCredentialFlow
    {
        public static void Run()
        {
            try
            {
                ConfigUtils.AuthenticationConfig config =
                    ConfigUtils.ReadFromJsonFile<ConfigUtils.AuthenticationConfig>("resources/secret.json");
        ...
        private static async Task RunAsync(ConfigUtils.AuthenticationConfig config)
        {
            ...
            string[] scopes = {$"{string.Join(",", config.Scopes)}"};
            // string[] scopes = {$"{config.Scopes[0]}"};
            ...
            if (result != null)
            {
                ...
                await apiCaller.CallWebApiAndProcessResultASync($"{config.Scopes}v1.0/users", result.AccessToken,
                    Display);
            }
        }
        ...
        /// <returns></returns>
        private static bool AppUsesClientSecret(ConfigUtils.AuthenticationConfig config)
        {
